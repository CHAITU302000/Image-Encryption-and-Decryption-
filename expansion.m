function key=expansion(key)
       ...
       for i=5:44
           ...
           if mod(i-1,4)==0
              ...
              %for k=1:4
                 for j=1:4
                     row=uint8(bitsra(key_1(j,i-1),4))+1;
                     col=uint8(mod(key_1(j,i-1),16))+1;
                     key_1(j,i-1)=((S_box(row,col)));
                 end
              %end

               i;
               key(1:4,i)=bitxor(key_1(:,i-1),key(:,i-4));
           else
               key(1:4,i)=bitxor(key(:,i-1),key(:,i-4));
           end
       end
       key
end
